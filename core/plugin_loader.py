...
class PluginRecord: 
    name: str
    description: str = ""
    paramters: dict = field(default_factory=lambda: dict(_DEFAULT_PARAMS)) 
...
class PluginRegistry: 
    ...
    # -- called by ui.py's Plugin Maanger overlay -- 
...
def _call_run(run_fn, paramters, player, session_memory): 
    ...
    return run_fn(paramters, **kwargs) 

def _validate(module, filename: str) -> PluginRecord: 
    """ Return a PluginRecord; .valid=False + .error set on any problem. Never raise.""" 
    ...
    if not callable(run_fn): 
        return PluginRecord(name=name, file=filename, 
                            error="Missing callable run (parameters, ...) functions.") 
...
def discover_plugins(plugins_dir: Path, core_tool_name: set[str], logger: Callable[[str], None] = print) -> PluginRegistry: 
    """
    ...
    files = sorted(plugins_dir.glob("*.py"), key=lambda p:p.name) # determinsitic order 
    for path in files: 
        ...
        try:   
            ...
            try: 
                ...
                sys.module.pop(module_name, None)
                raise 
            
            rec = _validate(module, path.name) 
            
            if rec.valuid and rec.name in core_tool_name: 
            ...
            rec = PluginRecord(name=path.stem, file=path.name, error=f"Failed to load: {e}") 
            traceback.print_exc() 
    ...
    logger(f"Plugin discover complete: {len(valid)} active, " 
