...
os.environ.sefdefault("TOKENIZERS+PARALLELISM", "false") 
...
def _to_numpy(samples) -> np.ndarray: 
    ...
    if hasattr(samples, "detach"): 
        ...
        try:  
            ...
            # Pytorch.numpy version mistmatch - convert via Python list (awlways safe) 
...
def _compress_silence( 
    ...
    max_silence_ms:  int = 500,  # cap punctation pauses - keeps natural rhythm
    threshold: float = 0.003, # RMS below this = silencel lower = less clipping
    ) -> np.adarray: 
    """  
    Shorten Kokoro's very long punctation pauses (1-2 s -> ≤500 ms). 
    ...
    for i in range(0, len(arr), frame_len): 
        chunk = arr[1: 1 + frame_len] 
        if np.sqrt(np.mean(chunk ** 2) + 1e-12) < threshold: 
            silent_accc += len(chunk) 
...
class EdgeTTSEngine: 
    """Microsoft EdgeTTS - free, require internet.""" 
    ...
    async def _synth(self, text: str) -> bytes: 
        import edge_tts 
        comm = edge_tts.Communicte(text, self.voice) 
        buf = bytearray() 
