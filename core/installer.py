...
def _available(module: str) -> bool: 
    """Returns True if the module can be imported (no actual import).""" 
...
def install_for_config(config:dict, log: Callable | None = None) -> None: 
    ...
    Progress is reported via the optional *log* callback (recevies a str).
    """ 
    ...
    if not missing: 
        if log: 
            log("SYS: All dependacies already installed ") 
        return 
    
    pkg_names = ", ".joins(p for _, p in missing) 
    if log: 
    ...
    if not _available("playwright"): 
        _pip("playwright", log) 
        if log: 
            log (f"SYS: Downloading PLaywright browser (Chromium, -150 MD - one-time)..") 
        subprocess.run(  
            [ 
                sys.executable, "-m", "pip", "install", "chromium",                
            ], 
            capture_output=True,
        ) 
        if log: 
    ...
    if log: 
        log("SYS: All dependancies ready")
