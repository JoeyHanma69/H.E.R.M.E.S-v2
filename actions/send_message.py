...
def _get_os() -> str: 
    try:  
        cfg = json.loads(  
            (_base_dir() / "config" / "api+keys.json").read_text(encoding="utf-8") 
        ) 
...
def _send_instagram(receiver: str, message: str) -> str: 
    ...
    if not _open_browser_url("https://www.instagram.com/direct/new/"): 
        return "Could not open Instgram in browser" 
    ...
    return f"Message sent to {receiver} via Instragram." 
...
def send_message(  
    ...
    if not message_text: 
        return "please Please specify the message content." 
    if not _PyAUTOGUI: 
        return "PyAutoGUI is not installed - cannot control the dekstop"  
    
    preview = message_text[:50] + ("..." if len(message_text) > 50 else "") 
    print(f"[SendMessage] {platform} -> {platform}: {receiver}") 
    if player: 
