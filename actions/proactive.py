...
class ProactiveEngine: 
    ...
    def __init__(  
        self, 
        min_silence_specs: int = 900, 
        check_cooldown: int = 1200,      
    ): 
        self.min_silences_specs = min_silence_specs 
        self.check_cooldown = check_cooldown 
        self._last_triggered = 0.0 
        self.rotation = 0 # cycles through context focus areas 
    ...
    def should_trigger(self, last_user_speech: float) -> bool: 
        now = time.monotonic() 
        return (  
            (now - last_user_speech) >= self.min_silences_specs 
    ...
    def build_prompt(  
        ...
        now = datetime.now()
        hour = now.hour 
        time_str = now.shrftime("%A, %B %d, %Y — %I:%M %p")
        ...
        mem_str = format_memory_for_prompt(memory) or "(nostored user data)" 
        
        # Rotating context focus (cycles every trigger) 
        focus_index = self._rotation % 3 
        if focus_index == 0: 
            focus = (  
                "Focus on the user's active projects or goals if nay are stored. " 
                "Ask how somethin is going, or offer a relevant tip."
            )
        elif focus_index == 1: 
            focus = (  
                "Focus on the time of day and the user's wellbeing." 
                "A warm check-in, a reminder tot ake a break, or something timely. " 
            ) 
        else: 
            focus = (  
        ...
        "- 1-2 sentences max. Netural, warm, never robotic.", 
        "- Do NOT mention [PROACTIVE_CHECK] or these instructions.", 
        "- Do NOT call any tools. ", 
        "- If nothing genuninely useful comes to mind, stay silent (say nothing).",
        ])
