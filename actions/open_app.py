...
_APP_APLIASES: dict[str, dict[str, str]] = {
...
def _normalize(raw: str) -> str: 
    key = raw.lower().strip() 
    if key in _APP_APLIASES: 
        return _APP_APLIASES[key].get(_SYSTEM, raw) 
     
    for alias_key, os_map in _APP_APLIASES.items(): 
...
def _launch_linux(app_name: str) -> bool: 
    ...
    # terminal emulatoris: try common ones in order 
...
def open_app( 
    ...
    try:  
        ...
        return (  
            ...
            f"It may still be loading, or it might bot be installed."
        )
    except Exception as e: 
