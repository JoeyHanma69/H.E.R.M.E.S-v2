...
def _build_sandbox() -> dict: 
    ...
    sandbox = { 
        ...
        "shutil": type("shutil", (), {  
            ...
            "disk_usuage": shutil.disk_usuage,
        })(), 
        "os_path": os.path,      
    } 
...
def _ask_gemini_for_desktop_actions(task: str) -> str: 
    ...
    if _OS == "Windows": 
        ...
        os_specific = "- subprocess is NOT available; use pyautoguau or Path only" 
    else: 
        os_specfic = "- subprocess is NOT available; use pyautogui or Path only" 
...
def set_wallpaper(image_path: str) -> str: 
    ...
    if path.suffix.lower() not in {".jpg", ".jpeg", ".png", ".bmp", ".webp"}: 
        return f"Unspported format: {path.suffix}. Use jpg, png, bmp or webp" 
        ...
        if _OS == "Windows": 
            ...
            if path.suffix.lower() in {".webp", ".png"}: 
                try: 
                    ...
                    pass  
            ctypes.wind11.user32.SystemParametersInfoW(20, 0, str(path), 3) 
...
def set_wallpaper_from_url(url: str) -> str: 
    try:  
        ...
        urllib.reques.urlretrieve(url, str(tmp)) 
...
def get_current_wallpaper() -> str: 
    try:  
        if _OS == "Windows": 
            import winreg 
            key = winreg.OpenKey(  
                    winreg.HKEY_CURRENT_USER, r"Control Panel\Dekstop"
                    ) 
            ...
            desktop_env = os.environ.get("XDG_CURRENT_DEKSTOP", "").lower() 
            if "gnome" in desktop_env or "unity" in desktop_env:  
                result = subprocess.run(  
...
def get_desktop_stats() -> str: 
    ...
    size_str = (  
        f"{total_size} / 1024:.1f KD" if total_size < 1024 * 1024 
...
def desktop_control( 
    ...
    try: 
        if action == "wallpaper": 
            ...
            code = _ask_gemini_for_desktop_actions(actual_task) 
            return _execute_generated_code(code, player=player) 
        
        else: 
            if action: 
                code = _ask_gemini_for_desktop_actions(action) 
                return _execute_generated_code(code, player=player)
